...
function Orden ({ orden }) {
  ...
  return (
    <div className='border p-10 space-y-5'>
      <h3 className='text-2xl font-blold'>Orden: {id}</h3>
      ...
      <div>
        {pedido.map(plato => (
          ...
          >
            <div className='w-32'>
              <Image width={400} height={500} src={`/img/${plato.imagen}.jpg`} alt={`Imagen del Plato ${plato.nombre}`} />
            </div>
      ...
      <div className='md:flex md:items-center md:justify-between my-10'>
        <p className='mt-50 font-black text-4xl text-amber-500'>Total a Pagar: {formatearDinero(total)}</p>
        <button
